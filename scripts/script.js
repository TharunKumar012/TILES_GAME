...
function setup() {
    ...
    playButton.addEventListener('click', () => {
        document.querySelector('img').style.visibility = 'hidden'
        document.querySelector('#message').style.color = `rgb(61, 61, 61)`
        document.querySelector('#message').textContent = `The sequence will get harder and you must click the tiles in the right order to advance to the next round.`
        gameOver = false;
        roundClicks = 0;
        playRound()
        allTiles.forEach(element => {
...
function clickTile(evt) {
    ...
    if (evt.target.getAttribute('id') === tileSequence.get(roundClicks)) {
        tile.style.backgroundColor = 'rgb(50, 255, 126)'
        playClickSfx()
        setTimeout(() => {
            tile.style.backgroundColor = 'rgb(61, 61, 61)'
        }, 100);
    } else {
        playSmackSfx()
        playPhoenixSfx()
        tile.style.backgroundColor = 'rgb(255, 56, 56)'
